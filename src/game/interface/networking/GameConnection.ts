...
export default class GameConnection extends Connection {
  ...
  constructor(logger: Logger, socket: any, logoutService: LogoutService, config: Config) {
    super(socket, logger);
  ...
  #onOtherCharacterMoved(otherCharacterMovedEvent) {
    const { virtualId, arg, duration, movementType, time, rotation, positionX, positionY } = otherCharacterMovedEvent;
    this.send(
  ...
  send(packet) {
    console.log("Enqueueing packet: ", packet);
