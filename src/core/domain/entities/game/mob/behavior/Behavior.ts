...
export default class Behavior {
    ...
    battleState() {
        ...
        if (!this.monster.getTarget() || this.monster.getTarget().isDead()) {
            return this.moveToOriginalPosition();
        }
        ...
        if (distanceFromTarget > this.monster.getAttackRange() * 1.15) {
            if (this.nextMoveTime <= now) {
                this.followTarget();
                this.nextMoveTime = now + MIN_NEXT_TIME_TO_MOVE;
            }
            return;
        }
    ...
    private findNextVictim(): boolean {
        for (const entity of this.monster.getNearbyEntities().values()) {
            ...
            if (distance <= this.monster.getAggressiveSight()) {
