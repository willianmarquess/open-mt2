...
export default class ItemMovePacketValidator extends PacketValidator<ItemMovePacket> {
    build() {
        this.createRule(this.packet.getFromWindow(), 'fromWindow').isRequired().isNumber().isGreaterThanOrEqual(0).build();
        ...
        this.createRule(this.packet.getToPosition(), 'toPosition').isRequired().isNumber().isGreaterThanOrEqual(0).build();
