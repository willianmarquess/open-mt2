...
export default class CreateCharacterPacketValidator extends PacketValidator<CreateCharacterPacket> {
    build() {
        ...
        this.createRule(this.packet.getPlayerClass(), 'playerClass').isRequired().isNumber().isGreaterThanOrEqual(0).build();
        this.createRule(this.packet.getAppearance(), 'appearance').isRequired().isNumber().isGreaterThanOrEqual(0).build();
    }
}
